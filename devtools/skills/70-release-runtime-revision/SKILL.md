...
description: Release or prepare an AXTP mock-server repository-only revision without changing the locked AXTP Spec. Use when the user asks to bump, publish, tag, or prepare a mock/server/package revision such as v0.8.4.1, to ship implementation, packaging, tool, generated mock asset, or documentation fixes while staying on the current AXTP_SPEC.lock.yaml version. Do not use for AXTP Spec releases or protocol fact changes.
---

# Release Runtime Revision

Prepare a repository-only revision for the AXTP Spec version already locked by this repository. Release tags use `vX.Y.Z.R`: `X.Y.Z` is owned by the AXTP Spec repo, and `R` is owned by this repository.
...
- If the AXTP Spec repo publishes `spec/vX.Y.Z`, use the spec upgrade workflow instead; it resets the repository version to `X.Y.Z.0`.
...
gh workflow run release-runtime-revision.yml \
  -f reason="Describe the repository-only change"
```
...
The workflow opens a PR that bumps only the repository revision and regenerated version metadata. After that PR merges, the existing auto-release workflow creates and pushes tag `vX.Y.Z.R`.

## Local Preparation Path

Work from the repository root. Ensure the locked spec checkout is available:
...
Run repository build and test commands before merging the PR. The release tag must be created by the existing auto-release workflow after merge, not by this skill directly.
