...
Prepare a repository-only release revision for the AXTP Spec version locked in
AXTP_SPEC.lock.yaml. If --target-version is omitted, increment the current
repository revision by one.
USAGE
}
...
parse_version "$current_version" "Current repository version"
...
if [[ "$current_spec_version" != "$spec_version" ]]; then
  echo "Current repository version $current_version does not match locked AXTP Spec version $spec_version" >&2
  exit 2
fi
...
parse_version "$target_version" "Target repository version"
...
if [[ "$target_spec_version" != "$spec_version" ]]; then
  echo "Target repository version $target_version must stay on locked AXTP Spec version $spec_version" >&2
  exit 2
fi
if [[ -z "$target_revision" || "$target_revision" == "0" ]]; then
  echo "Repository-only releases must use a positive fourth revision field: $target_version" >&2
  exit 2
fi
if (( target_revision <= current_revision )); then
  echo "Target repository revision $target_revision must be greater than current revision $current_revision" >&2
  exit 2
fi
if [[ ! -d "$spec_path" ]]; then
...
if [[ "$spec_lock_hash_after" != "$spec_lock_hash_before" ]]; then
  echo "AXTP_SPEC.lock.yaml changed during a repository-only revision release" >&2
  exit 1
fi
...
cat <<EOF
Prepared $runtime_name repository revision:
  AXTP Spec: $spec_tag ($spec_version)
  Previous repository version: $current_version
  Target repository version: $target_version
EOF
